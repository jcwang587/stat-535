...
(gender_char <- c("m", "m", "m"))
(gender_factor <- factor(gender_char, levels = c("m", "w")))

table(gender_char)
table(gender_factor)

gender_factor[4] <- "non-binary"
gender_factor
table(gender_factor)
table(gender_factor, useNA = "ifany")

# No longer in the 1980s:
levels(gender_factor) <- c(levels(gender_factor), "non-binary")
gender_factor

gender_factor[4] <- "non-binary"
gender_factor[5] <- "typo"

# Go back to the 1980s:
levels(gender_factor) <- c("m", "w")

length(gender_factor) <- 3
levels(gender_factor) <- factor(gender_factor)
gender_factor
...
read_csv()
read.csv() # is more flexible with handling handling encoding systems e.g. UTF-16
...
levels(x) <- gsub(levels(x), pattern = "a", replacement = "c")
