...
for (i in 1:nrow(models)) {
  lines(0:100, models$a1[i] + models$a2[i] * sqrt(0:100), col = rgb(0, 0, 0, 0.3))
}
...
sim$y - model1(7, 1.5, sim)

measure_distance1 <- function(a1, a2, data) {
  diff <- data$y - model1(a1, a2, data)
...
models$dist <- mapply(measure_distance1, a1 = models$a1, a2 = models$a2, MoreArgs = list(data = sim))

# Or equivalently using `purrr::map2dbl()`

models <- models %>% 
  mutate(dist2 = purrr::map2_dbl(a1, a2, .f = measure_distance1, data = sim))
...
measure_distance1(1, 2, sim)
measure_distance1_helper(c(1, 2), data = sim)
...
ggplot(sim, aes(x, y)) +
  ...
  geom_function(fun = function(x) best1$par[1] + best1$par[2] * sqrt(x), color = "blue")
