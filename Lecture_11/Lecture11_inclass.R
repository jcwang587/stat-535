...
# As a matter of fact, this is one of the reasons why C++ comiled code is faster: less overheads whenever we call a function. 

# A few more points about coding in C++:
# 1. Every line in C++ code has to end with a semicolon `;`
# 2. Every variable has to be decalred before it may be used:
...
# (what is the slight difference in declaration style betweem the integer and float versions?)
...
# C++: ++i is equivanelt to i <- i + 1 in R
