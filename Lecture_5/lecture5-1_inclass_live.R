# Lecture 5 - Part I - Data transformation
...
(jan_1st_flights <- filter(flights, month == 1, day == 1))
(jan_1st_flights_v2 <- filter(flights, month == 1 & day == 1))
# Moral: the "," translates to "&"

identical(jan_1st_flights, jan_1st_flights_v2)

all(jan_1st_flights$month == 1)
all(jan_1st_flights$day == 1)
...
sanity_check1 <- 
  ...
  mutate(sanity_check_dep_delay = dep_time - sched_dep_time)

sanity_check1$sanity_check_dep_delay == sanity_check1$dep_delay

# Valid method:
View(filter(flights, !(dep_delay > 120 | arr_delay > 120)))
...
# trailing spaces, leading spaces, lowercase vs. uppercase, NA values, encoding issues, values out of scope
...
select(flights, year_of_13 = year, year_of_12 = year, year, month, day)
...
var_name <- "month"
select(flights, var_name)
select(flights, all_of(var_name))
...
flights %>% transmute(lagged_dep_delay = lag(dep_delay), dep_delay)
...
transmute(flights, standardized_dep_delay = (dep_delay - mean(dep_delay, na.rm = TRUE)) / sd(dep_delay, na.rm = TRUE))
...
# Exercise =====
# Write a `for` loop that performs the same task as above.
res <- numeric(1 * 12 * 31)
i <- 1
for (y in 2013) {
  for (m in 1:12) {
    for(d in 1:31) {
      res[i] <- mean(flights$dep_delay[flights$year == y & flights$month == m & flights$day == d], na.rm = TRUE)
      i <- i + 1
    }
  }
}
...
by_day_1 <- group_by(flights, year, month, day)
(by_day_1_s1 <- summarise(by_day_1, delay = mean(dep_delay, na.rm = TRUE)))
summarize(by_day_1_s1, delay = mean(delay, na.rm = TRUE))

by_day_2 <- group_by(flights, day, year, month)
by_day_2_s1 <- summarise(by_day_2, delay = mean(dep_delay, na.rm = TRUE))
summarize(by_day_2_s1, delay = mean(delay, na.rm = TRUE))
...
# amounts of data. This is demonstrated in the following example for the pipe.
...
# to check that you’ve done the manipulation correctly.
